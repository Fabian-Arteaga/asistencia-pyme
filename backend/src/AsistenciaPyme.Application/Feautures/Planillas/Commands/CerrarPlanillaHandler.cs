...
public class CerrarPlanillaHandler : IRequestHandler<CerrarPlanillaCommand, PlanillaDto?>
{
    private readonly IAsistenciaPymeDbContext _context;

    public CerrarPlanillaHandler(IAsistenciaPymeDbContext context)
    {
        _context = context;
    }

    public async Task<PlanillaDto?> Handle(CerrarPlanillaCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("La planilla no puede cerrarse en el estado actual.");
        }

        if (planilla.Estado != EstadoPlanilla.EnRevision && planilla.Estado != EstadoPlanilla.Calculada)
        {
            throw new InvalidOperationException("Solo una planilla calculada o en revisión puede cerrarse.");
        }
        ...
        planilla.CerradoPor = administrador.IdAdministrador;
